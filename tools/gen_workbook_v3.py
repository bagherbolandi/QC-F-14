...
LIST_NAME={'DIM_SHIFT':'shift_list','DIM_STATION':'station_list','DIM_PART':'part_list',
           ...
           'DIM_STATUS_NEW':'status_new_list'}
...
def build_helper(ws, pl, n_rows, cap):
    ...
    params=[('N_ROWS',n_rows),('CAP_ROWS',cap),('FIRST_ROW',2),('LAST_ROW',1+n_rows),
            ...
            ('VERSION','v3.0.0'),('SHEET_DATA',SH['data'])]
...
def build_settings(ws, pl):
    ...
    r+=1
    c1=r
    for key,val in (('DATA',SH['data']),('FORM',SH['form']),('STAGING',SH['staging']),('MASTER',SH['master']),('HELPER',SH['helper']),('LOG',SH['log']),('DASH',SH['dash']),('HELP',SH['help'])):
...
def build(path, pl):
    ...
    sheets={}
    for k in ('form','staging','master','helper','reports','dash','dq','set','help','log','verify'):
    ...
    wb.properties.title='QC-F-14 v3 — Record_ID + Record_Type + Result + Disposition + Status'
    wb.properties.creator='QC-F-14 v3'
...
def main():
    ...
    ap.add_argument('--payload', default='analysis/payload_v3.json')
    ap.add_argument('-o','--out', default='QC-F-14-v3.xlsx')
    a=ap.parse_args()
